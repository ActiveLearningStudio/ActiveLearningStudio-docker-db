...
db_backup(){
	...
	### Start database backups
		...
		DATABASES=`$MYSQL $CREDENTIALS -h $MYSQL_HOST -P $MYSQL_PORT --ssl-mode=DISABLED -Bse 'show databases' | grep -Ev "^(Database|mysql|performance_schema|information_schema)"$`
        else
		DATABASES=$DB_NAMES
        fi
        ...
        do
                ...
                ${MYSQLDUMP} ${CREDENTIALS} -h ${MYSQL_HOST} -P $MYSQL_PORT --ssl-mode=DISABLED $db | ${GZIP} -9 > $FILENAMEPATH
...
check_mysql_connection(){
        ${MYSQLADMIN} ${CREDENTIALS} -h ${MYSQL_HOST} -P ${MYSQL_PORT} --ssl-mode=DISABLED ping | ${GREP} 'alive'>/dev/null
